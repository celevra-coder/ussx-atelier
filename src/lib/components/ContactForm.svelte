...
if (!acceptGdpr) errs.acceptGdpr = 'Необходимо е съгласие за обработка на данни';
		...
		if (firstError) {
			// Bring the first problem field into view and focus it
...
await fetch('/api/notify', {
	...
	const sectionClass =
		'pt-2 text-xs font-bold uppercase tracking-widest text-brand-blue-light';
</script>
...
<section id="contact" class="py-24">
	<div class="mx-auto max-w-2xl px-4 sm:px-6">
		...
		{#if submitted}
			<div
				class="reveal card-gradient-border p-10 text-center"
				use:inview
			>
				<div
				...
				<button onclick={reset} class="btn-outline text-sm"> Изпрати ново запитване </button>
			</div>
		{:else}
			<div class="reveal card-gradient-border p-6 sm:p-8" use:inview>
				...
				<form onsubmit={handleSubmit} novalidate>
					{#if type === 'course'}
						<!-- Course form -->
						<div class="space-y-5">
							<p class={sectionClass}>1. Данни за родителя</p>
							...
							<div class="space-y-3 border-t pt-4" style="border-color: rgba(110,198,245,0.1);">
								...
								<label class="flex cursor-pointer items-start gap-3">
									...
									<span class="text-sm text-brand-muted">
										Разбирам, че плащането се извършва <strong class="text-brand-text">на място</strong> при явяване на курса. *
									</span>
								</label>
...
{/if}
							...
							<div class="border-t pt-4" style="border-color: rgba(110,198,245,0.1);">
								<label class="flex cursor-pointer items-start gap-3">
									...
									<span class="text-sm text-brand-muted">
										Съгласен/а съм личните ми данни да бъдат обработвани от USSX за целите на
